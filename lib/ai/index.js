...
/**
 * Generate a concise summary of a chat suitable for appending to a CLAUDE.md file.
 * Focus: decisions made, current state, next actions. Under 200 words.
 *
 * @param {object[]} messages - Array of { role, content } from getMessagesByChatId
 * @returns {Promise<string>} Summary text, or empty string on failure
 */
async function autoSummary(messages) {
  if (!messages || messages.length === 0) return '';
  try {
    const model = await createModel({ maxTokens: 500 });

    // Build a compact transcript — cap at last ~20 messages to stay within context
    ...
    const systemPrompt =
      'You are summarizing a chat conversation so it can be appended to a persistent CLAUDE.md dashboard file. ' +
      'Focus on: decisions made, current state of the work, next actions, and any key facts worth remembering. ' +
      'Skip small talk. Skip pleasantries. Write in short bullet lines, one idea per line, maximum 200 words. ' +
      'Do NOT use em-dashes or en-dashes — use commas, colons, or periods instead. ' +
      'Format as plain markdown with short headings if helpful (no H1, use H3 or bullets).';

    const response = await model.invoke([
      ['system', systemPrompt],
      ['human', `Summarize this conversation for the persistent record:\n\n${transcript}`],
    ]);
    ...
    console.error('[autoSummary] Failed to generate summary:', err.message);
    return '';
  }
}

export { chat, chatStream, summarizeAgentJob, persistMessage, autoTitle, autoSummary };
