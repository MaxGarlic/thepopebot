...
import {
  ...
  updateChatTitle,
  toggleChatStarred,
  setChatBucket as dbSetChatBucket,
  setChatVaultPath as dbSetChatVaultPath,
  createChatSummary as dbCreateChatSummary,
  markSummaryVaultWritten as dbMarkSummaryVaultWritten,
} from '../db/chats.js';
import {
 ...
 * @param {object} [options] - { chatMode, bucket, vaultPath }
 * @returns {Promise<object>}
 */
export async function createChat(id, title = 'New Chat', options = {}) {
  const user = await requireAuth();
  return dbCreateChat(user.id, title, id, options);
}
...
/**
 * Set a chat's bucket (with ownership check).
 * @param {string} chatId
 * @param {string} bucket - e.g. 'SALES', 'general'
 * @returns {Promise<{success: boolean}>}
 */
export async function setChatBucket(chatId, bucket) {
  const user = await requireAuth();
  const chat = getChatById(chatId);
  if (!chat || chat.userId !== user.id) {
    return { success: false };
  }
  dbSetChatBucket(chatId, bucket);
  return { success: true };
}

/**
 * Set a chat's vault path (with ownership check).
 * @param {string} chatId
 * @param {string|null} vaultPath - e.g. 'SALES/generals-recruitment/vince-citro/CLAUDE.md'
 * @returns {Promise<{success: boolean}>}
 */
export async function setChatVaultPath(chatId, vaultPath) {
  const user = await requireAuth();
  const chat = getChatById(chatId);
  if (!chat || chat.userId !== user.id) {
    return { success: false };
  }
  dbSetChatVaultPath(chatId, vaultPath);
  return { success: true };
}

/**
 * Generate and persist a chat summary, optionally writing to the vault file.
 * Used by both the /save manual command and the finalize-chat auto-summary.
 * @param {string} chatId
 * @param {'finalize'|'manual'} trigger
 * @returns {Promise<{success: boolean, summary?: string, vaultWritten?: boolean, vaultPath?: string|null}>}
 */
export async function saveChatSummary(chatId, trigger = 'manual') {
  const user = await requireAuth();
  const chat = getChatById(chatId);
  if (!chat || (chat.userId !== user.id && chat.userId !== 'telegram')) {
    return { success: false };
  ...
  // Generate summary via configured LLM
  let summaryText;
  try {
    const { autoSummary } = await import('../ai/index.js');
    summaryText = await autoSummary(messages);
  } catch (err) {
    console.error('[saveChatSummary] autoSummary failed:', err.message);
  ...
  // Persist to DB
  const summaryRow = dbCreateChatSummary(chatId, summaryText, trigger, chat.vaultPath);

  // Attempt vault write (fail-soft — DB record stays, vault_written stays 0 on failure)
  let vaultWritten = false;
  if (chat.vaultPath) {
    try {
      const { appendSummaryToVault } = await import('../vault-write.js');
      await appendSummaryToVault(chat.vaultPath, chat.title, summaryText, trigger);
      dbMarkSummaryVaultWritten(summaryRow.id);
      vaultWritten = true;
    } catch (err) {
      console.error('[saveChatSummary] vault write failed:', err.message);
    }
  }

  return { success: true, summary: summaryText, vaultWritten, vaultPath: chat.vaultPath };
}
