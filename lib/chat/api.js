...
/**
 * POST handler for /chat/[chatId]/set-bucket — assign a bucket to a chat.
 */
export async function setBucketHandler(request, { params }) {
  const session = await auth();
  if (!session?.user?.id) {
    return Response.json({ error: 'Unauthorized' }, { status: 401 });
  }
  const { id: chatId } = await params;
  const { bucket, vaultPath } = await request.json();
  const { setChatBucket, setChatVaultPath } = await import('./actions.js');
  const bucketResult = await setChatBucket(chatId, bucket);
  if (!bucketResult.success) return Response.json(bucketResult, { status: 403 });
  if (vaultPath !== undefined) {
    await setChatVaultPath(chatId, vaultPath || null);
  }
...
/**
 * POST /chat/[id]/save-summary — manually trigger summary generation + vault write.
 * Used by the /save command in chat and by "End Chat" actions.
 */
export async function saveSummaryHandler(request, { params }) {
  const session = await auth();
  if (!session?.user?.id) {
    return Response.json({ error: 'Unauthorized' }, { status: 401 });
  }
  ...
  try {
    const { saveChatSummary } = await import('./actions.js');
    const result = await saveChatSummary(chatId, trigger);
    return Response.json(result);
  } catch (err) {
    console.error('[saveSummaryHandler]', err);
