...
import { chats, messages, chatSummaries } from './schema.js';
 ...
 */
export function createChat(userId, title = 'New Chat', id = null, options = {}) {
  const db = getDb();
  const now = Date.now();
  const chat = {
    id: id || randomUUID(),
    userId,
    title,
    chatMode: options.chatMode || 'agent',
...
/**
 * Set a chat's bucket (top-level organization).
 * @param {string} chatId
 * @param {string} bucket - e.g. 'SALES', 'CLIENTS', 'general'
 */
export function setChatBucket(chatId, bucket) {
  const db = getDb();
  db.update(chats)
    .set({ bucket: bucket || 'general', updatedAt: Date.now() })
    .where(eq(chats.id, chatId))
    .run();
}

/**
 * Set a chat's vault path — relative path under AREAS/ where summaries write.
 * @param {string} chatId
 * @param {string|null} vaultPath - e.g. 'SALES/generals-recruitment/vince-citro/CLAUDE.md'
 */
export function setChatVaultPath(chatId, vaultPath) {
  const db = getDb();
  db.update(chats)
    .set({ vaultPath: vaultPath || null, updatedAt: Date.now() })
    .where(eq(chats.id, chatId))
    .run();
}

/**
 * Insert a chat summary record.
 * @param {string} chatId
 * @param {string} summary
 * @param {'finalize'|'manual'} trigger
 * @param {string|null} vaultPath
 * @returns {object} The created summary row
 */
export function createChatSummary(chatId, summary, trigger = 'finalize', vaultPath = null) {
  const db = getDb();
  const row = {
    id: randomUUID(),
    chatId,
  ...
  db.insert(chatSummaries).values(row).run();
  return row;
}

/**
 * Mark a summary as written to the vault file successfully.
 * @param {string} summaryId
 */
export function markSummaryVaultWritten(summaryId) {
  const db = getDb();
  db.update(chatSummaries)
    .set({ vaultWritten: 1 })
    .where(eq(chatSummaries.id, summaryId))
    .run();
}

/**
 * Get summaries for a chat, newest first.
 * @param {string} chatId
 * @returns {object[]}
 */
export function getChatSummaries(chatId) {
  const db = getDb();
  return db
    .select()
    .from(chatSummaries)
