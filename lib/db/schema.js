...
// Persisted chat summaries — written when a chat finalizes or user triggers /save
export const chatSummaries = sqliteTable('chat_summaries', {
  id: text('id').primaryKey(),
  chatId: text('chat_id').notNull(),
  summary: text('summary').notNull(),
  trigger: text('trigger').notNull().default('finalize'),
  vaultPath: text('vault_path'),
  vaultWritten: integer('vault_written').notNull().default(0),
  createdAt: integer('created_at').notNull(),
});
